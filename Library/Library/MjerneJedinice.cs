...
namespace Library
{
    public partial class MjerneJedinice
    {
        ...
        public bool CheckIfExist(string naziv)
        {
            ...
            using (SQLiteConnection cn = new SQLiteConnection(Helpers.SqLite.DBConnectionString))
            {
                ...
                using (SQLiteCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandText = String.Format("SELECT Id FROM JedinicaMjere WHERE Naziv='{0}';", naziv);
        ...
        public MjerneJedinice GetData(object criteria)
        {
            using (SQLiteConnection cn = new SQLiteConnection(Helpers.SqLite.DBConnectionString))
            {
                cn.Open();
        ...
        private void ExecuteFetch(SQLiteConnection cn, object criteria)
        {
            using (SQLiteCommand cm = cn.CreateCommand())
            {
                cm.CommandText = String.Format("SELECT * FROM JedinicaMjere WHERE Id='{0}'", (string)criteria);
                cm.CommandType = CommandType.Text;
        ...
        private void ExecuteInsert(SQLiteConnection cn)
        {
            ...
            {
                cm.CommandType = CommandType.Text;
                cm.CommandText = @"INSERT INTO JedinicaMjere (
								Naziv
							) VALUES (
								@Naziv
							);";
        ...
        private void ExecuteUpdate(SQLiteConnection cn)
        {
            ...
            {
                cm.CommandType = CommandType.Text;
                cm.CommandText = @"UPDATE JedinicaMjere SET
        ...
        private void ExecuteDelete(SQLiteConnection cn, object criteria)
        {
            ...
            {
                cm.CommandType = CommandType.Text;
                cm.CommandText = string.Format(@"DELETE FROM JedinicaMjere WHERE Id={0};", (int)criteria);
    ...
    public partial class MjerneJediniceList : CollectionBase
    {
        public MjerneJedinice this[int idx]
        {
            get
            {
                return (MjerneJedinice)this.InnerList[idx];
            }
        }
        ...
        public MjerneJediniceList GetData(object criteria)
        {
            using (SQLiteConnection cn = new SQLiteConnection(Helpers.SqLite.DBConnectionString))
            {
                cn.Open();
        ...
        private void ExecuteFetch(SQLiteConnection cn, object criteria)
        {
            using (SQLiteCommand cm = cn.CreateCommand())
            {
                ...
                cm.CommandType = CommandType.Text;
                cm.CommandText = "SELECT * FROM JedinicaMjere";
                if (criteria != null)
                {
                ...
                using (Helpers.SafeDataReader dr = new Helpers.SafeDataReader(cm.ExecuteReader()))
                {
                    ...
                    while (dr.Read())
                    {
                        MjerneJedinice item = new MjerneJedinice();
