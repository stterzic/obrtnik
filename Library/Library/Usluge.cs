...
namespace Library
{
    public partial class Usluge
    {
        ...
        public bool CheckIfExist(string naziv)
        {
            ...
            using (SQLiteConnection cn = new SQLiteConnection(Helpers.SqLite.DBConnectionString))
            {
                ...
                using (SQLiteCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandText = String.Format("SELECT Id FROM Usluge WHERE Naziv='{0}';", naziv);
        ...
        public Usluge GetData(object criteria)
        {
            using (SQLiteConnection cn = new SQLiteConnection(Helpers.SqLite.DBConnectionString))
            {
                cn.Open();
        ...
        private void ExecuteInsert(SQLiteConnection cn)
        {
            ...
            {
                cm.CommandType = CommandType.Text;
                cm.CommandText = @"INSERT INTO Usluge (
								Naziv
							) VALUES (
								@Naziv
							);";
        ...
        private void ExecuteUpdate(SQLiteConnection cn)
        {
            ...
            {
                cm.CommandType = CommandType.Text;
                cm.CommandText = @"UPDATE Usluge SET
    ...
    public partial class UslugeList : CollectionBase
    {
        public Usluge this[int idx]
        {
            get
            {
                return (Usluge)this.InnerList[idx];
            }
        }
        ...
        public UslugeList GetData(object criteria)
        {
            using (SQLiteConnection cn = new SQLiteConnection(Helpers.SqLite.DBConnectionString))
            {
                cn.Open();
        ...
        private void ExecuteFetch(SQLiteConnection cn, object criteria)
        {
            using (SQLiteCommand cm = cn.CreateCommand())
            {
                ...
                cm.CommandType = CommandType.Text;
                cm.CommandText = "SELECT * FROM Usluge";
                if (criteria != null)
                {
