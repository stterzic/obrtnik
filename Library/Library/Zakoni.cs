...
namespace Library
{
    public partial class Zakoni
    {
        ...
        public bool CheckIfExist(string naziv)
        {
            ...
            using (SQLiteConnection cn = new SQLiteConnection(Helpers.SqLite.DBConnectionString))
            {
                ...
                using (SQLiteCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandText = String.Format("SELECT Id FROM Zakoni WHERE Naziv='{0}';", naziv);
        ...
        public Zakoni GetData(object criteria)
        {
            using (SQLiteConnection cn = new SQLiteConnection(Helpers.SqLite.DBConnectionString))
            {
                cn.Open();
        ...
        private void ExecuteFetch(SQLiteConnection cn, object criteria)
        {
            using (SQLiteCommand cm = cn.CreateCommand())
            {
                cm.CommandText = String.Format("SELECT * FROM Zakoni WHERE Id='{0}'", (string)criteria);
                cm.CommandType = CommandType.Text;
        ...
        private void ExecuteInsert(SQLiteConnection cn)
        {
            ...
            {
                cm.CommandType = CommandType.Text;
                cm.CommandText = @"INSERT INTO Zakoni (
								Naziv,
                                Napomena
							) VALUES (
								@Naziv,
        ...
        private void ExecuteUpdate(SQLiteConnection cn)
        {
            ...
            {
                cm.CommandType = CommandType.Text;
                cm.CommandText = @"UPDATE Zakoni SET
								Naziv@Naziv,
                                Napomena=@Napomena WHERE Id=@Id;";
        ...
        private void ExecuteDelete(SQLiteConnection cn, object criteria)
        {
            ...
            {
                cm.CommandType = CommandType.Text;
                cm.CommandText = string.Format(@"DELETE FROM Usluge WHERE Id={0};",(int)criteria);
    ...
    public partial class ZakoniList : CollectionBase
    {
        public Zakoni this[int idx]
        {
            get
            {
                return (Zakoni)this.InnerList[idx];
            }
        }
        ...
        public ZakoniList GetData(object criteria)
        {
            using (SQLiteConnection cn = new SQLiteConnection(Helpers.SqLite.DBConnectionString))
            {
                cn.Open();
        ...
        private void ExecuteFetch(SQLiteConnection cn, object criteria)
        {
            using (SQLiteCommand cm = cn.CreateCommand())
            {
                ...
                cm.CommandType = CommandType.Text;
                cm.CommandText = "SELECT * FROM Zakoni";
                if (criteria != null)
                {
